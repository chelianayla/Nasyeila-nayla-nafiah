...
export type DashboardDataMode = 'none' | 'budget_only' | 'gl_only' | 'both';
...
export interface DataUploadStatus {
  ...
  budgetUploadedAt?: string;
  glFileName?: string;
  glUploadedAt?: string;
  glMonths: FiscalMonth[];
...
export interface AccountVarianceSummary {
  ...
  budgetYtd: number;
  actualYtd: number;
  variance: number | null; // Actual - Budget (null when single file)
  variancePct: number | null; // (Variance / Budget) * 100
  absorptionRate: number | null; // (Actual / Budget) * 100
  annualBudget: number;
  ...
  priorYearVariancePct?: number | null;
  priorYearAbsorptionRate?: number | null;
